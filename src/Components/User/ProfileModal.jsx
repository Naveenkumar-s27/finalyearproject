...
import {
  UserRound, Package, Settings, LogOut, ChevronRight, X, MapPin,
  Phone, Mail, Edit2, Check, Camera, Lock, Eye, EyeOff, Bell,
  Truck, Tag, ShoppingCart, ChevronDown, ChevronUp,
} from "lucide-react";
...
const SettingsPanel = ({ user, onLogout }) => {
  ...
  const [section,   setSection]   = useState(null); // "password" | "notifications"
  const [pwForm,    setPwForm]    = useState({ current:"", newPw:"", confirm:"" });
  const [pwErr,     setPwErr]     = useState("");
  const [pwOk,      setPwOk]      = useState(false);
  const [showPw,    setShowPw]    = useState(false);
  const [notifs,    setNotifs]    = useState(() => {
    try { return JSON.parse(localStorage.getItem("vmart_notifs")) || { orders:true, offers:true, delivery:true }; }
    catch { return { orders:true, offers:true, delivery:true }; }
  });

  const saveNotifs = (updated) => {
    setNotifs(updated);
    localStorage.setItem("vmart_notifs", JSON.stringify(updated));
  };
  ...
  const handlePwSubmit = () => {
    ...
    localStorage.setItem("vmart_users", JSON.stringify(updated));
    setPwOk(true);
    setPwForm({ current:"", newPw:"", confirm:"" });
    setTimeout(()=>{ setPwOk(false); setSection(null); }, 2000);
  };

  const Toggle = ({ label, icon, field }) => (
    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0",borderBottom:"1px solid #f0f0f0" }}>
      <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
        <div style={{ color:"#006f1b" }}>{icon}</div>
        <span style={{ fontSize:"14px",color:"#333" }}>{label}</span>
      </div>
      <div
        onClick={() => saveNotifs({ ...notifs, [field]: !notifs[field] })}
        style={{ width:"42px",height:"24px",borderRadius:"12px",background:notifs[field]?"#006f1b":"#ddd",cursor:"pointer",position:"relative",transition:"background 0.2s" }}
      >
  ...
  return (
    <div>
      <h3 style={{ margin:"0 0 16px",fontSize:"14px",color:"#888",textTransform:"uppercase",letterSpacing:"0.5px" }}>
        Account Settings
      </h3>
            ...
            <div style={{ background:"#f9f9f9",border:"1px solid #eee",borderRadius:"8px",padding:"16px",marginBottom:"8px" }}>
                ...
                <div key={f.key} style={{ marginBottom:"12px" }}>
                  ...
                  <div style={{ position:"relative" }}>
                    <input
                      ...
                      placeholder={f.ph}
                      style={{ width:"100%",padding:"9px 12px",border:"1px solid #ddd",borderRadius:"6px",fontSize:"13px",outline:"none",boxSizing:"border-box",fontFamily:"'Poppins',sans-serif" }}
                    />
                  </div>
                </div>
              ))}
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"4px" }}>
                <label style={{ display:"flex",alignItems:"center",gap:"6px",fontSize:"12px",color:"#888",cursor:"pointer" }}>
                  <input type="checkbox" checked={showPw} onChange={()=>setShowPw(!showPw)}/> Show passwords
                </label>
                <button onClick={handlePwSubmit}
                  style={{ padding:"8px 16px",background:"#006f1b",color:"white",border:"none",borderRadius:"6px",cursor:"pointer",fontSize:"13px",fontWeight:600,fontFamily:"'Poppins',sans-serif" }}>
                  Update
                </button>
              </div>
            </div>
      ...
      <button onClick={onLogout}
        style={{ width:"100%",padding:"12px",marginTop:"16px",background:"#fff0f0",color:"#c0392b",border:"1px solid #f5c6cb",borderRadius:"8px",fontWeight:600,cursor:"pointer",fontSize:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",fontFamily:"'Poppins',sans-serif" }}>
...
const OrderCard = ({ order }) => {
  ...
  return (
    <div style={{ border:"1px solid #eee",borderRadius:"10px",marginBottom:"12px",overflow:"hidden" }}>
      <div style={{ padding:"14px",cursor:"pointer" }} onClick={()=>setOpen(!open)}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px" }}>
          <span style={{ fontWeight:700,fontSize:"14px" }}>{order.id}</span>
          <span style={{ padding:"3px 10px",borderRadius:"20px",fontSize:"12px",fontWeight:600,...(STATUS[order.status]||{ bg:"#f0f0f0",color:"#333" }) }}>
            {order.status}
          </span>
        </div>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div>
            <p style={{ margin:0,fontSize:"12px",color:"#888" }}>{order.date}</p>
            <p style={{ margin:"2px 0 0",fontSize:"12px",color:"#555" }}>{order.items} item{order.items!==1?"s":""}</p>
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:"8px" }}>
            <span style={{ fontWeight:700,color:"#006f1b",fontSize:"15px" }}>{order.total}</span>
          ...
          {(order.products||[]).map((p,i)=>(
            <div key={i} style={{ display:"flex",gap:"10px",alignItems:"center",marginBottom:"8px" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <p style={{ margin:0,fontSize:"12px",color:"#888" }}>×{p.qty} · {p.price}</p>
              </div>
            </div>
          ))}
          {order.address && (
            <div style={{ marginTop:"10px",padding:"10px",background:"#f0f7f0",borderRadius:"6px",fontSize:"12px",color:"#555" }}>
              <strong>Delivered to:</strong><br/>
              {order.address.name}, {order.address.houseNo} {order.address.street}, {order.address.city} - {order.address.pincode}
            </div>
          )}
        </div>
      )}
...
const ProfileModal = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <div style={{ position:"fixed",top:0,right:0,bottom:0,width:"400px",maxWidth:"95vw",background:"#fff",boxShadow:"-4px 0 20px rgba(0,0,0,0.15)",zIndex:1000,display:"flex",flexDirection:"column",fontFamily:"'Poppins',sans-serif",overflowY:"auto" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#005b16,#006f1b)",padding:"24px 20px 20px",color:"white",position:"relative",flexShrink:0 }}>
          ...
          {/* Order count badge */}
          <p style={{ margin:"6px 0 0",opacity:.65,fontSize:"12px" }}>{orders.length} order{orders.length!==1?"s":""} placed</p>
        ...
        <div style={{ display:"flex",borderBottom:"1px solid #eee",background:"#f9f9f9",flexShrink:0 }}>
          {[
            ...
            { key:"orders",   label:`Orders${orders.length?` (${orders.length})`:""}`, icon:<Package size={15}/> },
        ...
        <div style={{ flex:1,padding:"20px" }}>
          ...
          {tab==="profile" && (
            <>
              ...
              {!editing && <p style={{ textAlign:"center",color:"#ccc",fontSize:"12px",marginTop:"16px" }}>Tap Edit to update · Saved in your browser</p>}
            </>
          )}

          {/* ORDERS — real data from OrdersContext */}
          {tab==="orders" && (
            <>
              <h3 style={{ margin:"0 0 16px",fontSize:"14px",color:"#888",textTransform:"uppercase",letterSpacing:"0.5px" }}>Order History</h3>
              {orders.length === 0 ? (
                <div style={{ textAlign:"center",padding:"40px 0",color:"#888" }}>
                  <ShoppingCart size={40} style={{ opacity:.2,marginBottom:"12px" }}/>
                  <p>No orders yet</p>
                  <button onClick={()=>{ onClose(); }} style={{ marginTop:"12px",padding:"8px 18px",background:"#006f1b",color:"white",border:"none",borderRadius:"6px",cursor:"pointer",fontFamily:"'Poppins',sans-serif",fontWeight:600,fontSize:"13px" }}>
                    Start Shopping
                  </button>
                </div>
              ) : (
